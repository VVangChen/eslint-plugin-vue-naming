...
module.exports = {
  ...
  getAllowedProperties (componentObject) {
    const dataPropertiesNode = componentObject.properties
      .find(p =>
        p.type === 'Property' &&
        p.key.type === 'Identifier' &&
        p.key.name === 'data' &&
    ...
    const methodsPropertiesNode = componentObject.properties
      .find(p =>
        p.type === 'Property' &&
        p.key.type === 'Identifier' &&
        p.key.name === 'methods' &&
    ...
    if (!dataPropertiesNode && !computedPropertiesNode && !methodsPropertiesNode) { return [] }

    return [
      ...dataPropertiesNode.value.properties
        .filter(cp => cp.type === 'Property')
        .map(cp => {
          const key = cp.key.name
          let value
      ...
      ...computedPropertiesNode.value.properties
        .filter(cp => cp.type === 'Property')
        .map(cp => {
          const key = cp.key.name
          let value
      ...
      ...methodsPropertiesNode.value.properties
        .filter(cp => cp.type === 'Property')
        .map(cp => {
          const key = cp.key.name
          let value
  ...
  isVisMixinFile (node, path) {
    // console.log('---------------', node, path)
    const isVisMixinJsFile = path.endsWith('.js') && path.startsWith('mixin')
    return isVisMixinJsFile &&
      node.type === 'ExportDefaultDeclaration' &&
      node.declaration.type === 'ObjectExpression' &&
      // node.declaration.properties.some(prop => console.log(prop))
      node.declaration.properties.some(prop => prop.key.name === 'name' && prop.value.value.startsWith('mixin'))
  },

  executeOnVisMixin (context, cb) {
    const filePath = context.getFilename()
    const _this = this

    return {
      'ExportDefaultDeclaration:exit' (node) {
        // export default { name: 'mixin-' } in .js
        if (!_this.isVisMixinFile(node, filePath)) return
        cb(node.declaration)
      }
    }
  },
  ...
  * getAllOptions (node) {
    yield this.iterateObjectExpression(node)
  },
